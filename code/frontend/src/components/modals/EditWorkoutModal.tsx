...
const EditWorkoutModal = ({value, day, userId, currentCalories, onEditWorkoutSuccessful, onBackButtonClicked} : EditWorkoutProps) => {

    const { register, handleSubmit, formState: { errors, isSubmitting }} = useForm<NewWorkoutData>({
        mode: "onSubmit",
    ...
    return ( 
        <Form className={styles.edit_workout_form} onSubmit={handleSubmit(onSubmit)}>
                    <Button
                    ...
                    <p className={styles.current_calories}>Current Calories: {currentCalories}</p>
                    { 
                    ...
                    <input type="hidden" {...register("date")} defaultValue={getSubmitDate()}/>
