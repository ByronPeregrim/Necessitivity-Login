...
const EditInfoFormModal = ({currentUser, onEditInfoSuccessful, onBackButtonClicked}: EditInfoFormModalProps  ) => {
    const [errorText, setErrorText] = useState<string | null>(null);

    const { register, handleSubmit, formState: { errors, isSubmitting }} = useForm<EditUserInfoCredentials>({
        mode: "onSubmit",
        ...
        <Modal show>
            <Modal.Body className={styles.modal_body}>
                ...
                <Form className={styles.user_signup_form} onSubmit={handleSubmit(onSubmit)}>
                    <h1 className={styles.signup_message}>Edit {currentUser?.username}</h1>
                    { 
                    ...
                    <div className={styles.admin_box}>
                        <label htmlFor="admin"><b>Admin </b>Priveleges?</label>
                        <input type="checkbox" id="admin" {...register("admin")} />
                    </div>
                    <input type="hidden" {...register("oldUsername")} defaultValue={currentUser?.username}></input>
                    <input type="hidden" {...register("oldEmail")} defaultValue={currentUser?.email}></input>
